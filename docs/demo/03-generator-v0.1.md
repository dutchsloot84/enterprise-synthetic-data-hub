...
| `src/enterprise_synthetic_data_hub/models/person.py` & `vehicle.py` | Pydantic schemas enforcing GUID IDs, demographics, contact info, VIN/license formats, and `lob_type`. |
| `src/enterprise_synthetic_data_hub/generation/snapshot_generator.py` | Fabricates Persons and Vehicles, links them by `person_id`, injects metadata, and streams rows into structured payloads. |
| `src/enterprise_synthetic_data_hub/io/exporter.py` | Writes governed CSVs (`persons_v0_1.csv`, `vehicles_v0_1.csv`) and JSON bundles plus `snapshot_manifest_v0_1.json`. |
...
2. CLI loads dataset settings, seeds RNG, and invokes `snapshot_generator.generate_dataset()`.
3. Generator streams records through IO helpers, creating CSV + JSON outputs plus metadata.
4. Validators and tests confirm schema compliance before sharing the snapshot.
